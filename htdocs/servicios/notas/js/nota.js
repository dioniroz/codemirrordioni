...
setTimeout(function(){
	ajax.POST('notas.php/etiquetas').then((data)=>{
...
cambiarnota = function(){
  function cargarnota(valor){
     ajax.POST('notas.php/get',{id:valor}).then(function(response){
        res = response;
           if(res.editar=='true'){
      editar = res.editar;
...
fnguardar = function(){
  ...
  condicion = vid!=0;
  url = condicion ? 'notas.php/edit' : 'notas.php/set'
...
//insertando estilos en el documento
  document.head.append(r(`<style>
          .appnota{width:300px;height:200px;background:red;position:absolute;top:0;left:0;z-index:5;}
