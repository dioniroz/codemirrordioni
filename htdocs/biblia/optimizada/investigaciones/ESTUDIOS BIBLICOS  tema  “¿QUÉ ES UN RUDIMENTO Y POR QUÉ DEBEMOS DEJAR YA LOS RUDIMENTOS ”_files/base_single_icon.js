		...
		gotMessage: function(param, from)
		{
			...
			if (prep && prep.length)
			{
				...
				else if(param >= 200 && param < 2000)
				{
					...
					if (prep[6] && similarproducts.inimg && !similarproducts.inimg.hasTemplate())
					{
...
//								}
							...
							if( !sp.prodPage.e &&  sp.prodPage.s ) {
								if(similarproducts.b.inimgSrp && sp.prodPage.i == 0) {
                                    ...
                                    if (similarproducts.utilities.abTestUtil.getBucket() == "2014w09_NoCouponWithInimg_A") {
                                        //spsupport.log("gotMessage param="+param+" no results");
                                        sa.requestCouponsWl();
                                    }
...
//								}
				...
				else if (param > 2001)
				{
                    if (prep[3]) {
						...
						sfu.closePopup();
                        if (similarproducts.utilities.abTestUtil.getBucket() != '2014w13_Localv2_SeeMoreButton_EbayInImgSerget_Holdback' && sp.sgDualResults && sp.prodPage.p)
                        {
		...
		jQLoaded: function() {
            ...
            if (similarproducts.languages && similarproducts.languages[similarproducts.b.userData.lang] && similarproducts.utilities.abTestUtil.getBucket() != '2014w13_Localv2_SeeMoreButton_EbayInImgSerget_Holdback')
            {
            }    //language in test
            else
            {
			...
			if (!spsupport.sites.isBlackStage()) {
				...
				if (testBucket == 'uiRedesign01LT_A' || testBucket == 'uiRedesign01LT_B')
				{
		...
		injCpn: function(st) {  /* st - site type: 1 - wl, 2 - cwl, 3 - blws*/
			...
			if (similarproducts.utilities.abTestUtil && similarproducts.utilities.abTestUtil.getBucket() == '2014w07_NewCouponInfrastructure_A') {
			    ...
			    if( (similarproducts.b.cacheBySubDlsource || "" === "1") &&
			        (sp.CD_CTID || "" !== "") && (sp.CD_CTID || "" !== "-1") ){
				    url += "coupons/get.jsp?pi=" + sp.dlsource + "&psi="+ sp.CD_CTID + "&mc="+ sp.CD_CTID + "&ui=" + sp.userid + "&st="+ st1 + (similarproducts.b.CD_CTID ? "&cc="+ similarproducts.b.CD_CTID : "") + "&v=" + sp.appVersion;
			    } else {
		...
		onDocumentLoaded: function(obj)
		{
			...
			if( sS ) {
				...
				if (location.href.search(/similarproducts\.net\/window-shopper/i)) // Fix for similar-products.net demo page
		        {
			       // w.st = 1;
		        }
            ...
            if ((sfb.inimg && sp.pageType =="PP" && ifSg == 0 || sfb.inimgSrp && sp.pageType !="PP" && ifSg == 0) &&
                    (sp.siteType == 'wl' || sp.siteType == 'st' || sp.siteType == 'pip') &&
                     sp.prodPage.p &&
                     similarproducts.utilities.abTestUtil.getBucket() == "2014w09_NoCouponWithInimg_A") {
            }
            else {
		...
		addSimilarProductsSupport: function(){
			...
			if( !top.similarproducts.util && !spsupport.p.iSf)
			{
				...
				if (similarproducts.utilities.abTestUtil.getBucket() == 'uiRedesign01LT_A' || similarproducts.utilities.abTestUtil.getBucket() == 'uiRedesign01LT_B')
				{
					similarproducts.b.inj(window.document, similarproducts.b.site + "js/sf_allenby_redesign.js?version=" + spsupport.p.appVersion, 1);
				}
				else
				{
		...
		addSeeMoreButton: function()
		{
			...
			if (!similarproducts.b.buttonsTest)
			{
		...
		addAn: function(){
			...
			if(!sp.sfIcon.an)
			{
				...
				if (similarproducts.utilities.abTestUtil.getBucket() == '2014w13_Localv2_SeeMoreButton_EbayInImgSerget_Holdback')
				{
					similarproducts.b.buttonsTest = true;
				}
